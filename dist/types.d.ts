...
declare type AsParam<ParamName extends string> = {
    [P in ParamName]: string;
};
declare type AsOptParam<ParamName extends string> = {
    [P in ParamName]?: string;
};
/**
 ...
 */
export declare type ParseParams<A extends string> = A extends `:${infer Param}<${any}>/${infer Tail}` ? AsParam<Param> & ParseParams<Tail> : A extends `:${infer Param}<${any}>` ? AsParam<Param> : A extends `:${infer Param}/${infer Tail}` ? AsParam<Param> & ParseParams<Tail> : A extends `:${infer Param}` ? AsParam<Param> : A extends `;${infer Param}<${any}>/${infer Tail}` ? AsOptParam<Param> & ParseParams<Tail> : A extends `;${infer Param}<${any}>` ? AsOptParam<Param> : A extends `;${infer Param}/${infer Tail}` ? AsOptParam<Param> & ParseParams<Tail> : A extends `;${infer Param}` ? AsOptParam<Param> : A extends `${any}?:${infer Param}/${infer Tail}` ? AsOptParam<Param> & ParseParams<Tail> : A extends `${any}?:${infer Param}` ? AsOptParam<Param> : A extends `${any}?${infer Param}/${infer Tail}` ? AsOptParam<Param> & ParseParams<Tail> : A extends `${any}?${infer Param}` ? AsOptParam<Param> : A extends `*${infer Param}` ? {
    [P in Param]?: string[];
} : A extends `/${infer Tail}` ? ParseParams<Tail> : A extends `${any}/${infer Tail}` ? ParseParams<Tail> : {};
/**
 ...
 */
export declare type ReadRoutes<Tree extends RoutesLike<any>> = ReadRoutes__<Tree, [
], [
]>;
declare type ReadRoutes__<Tree, NameAcc extends string[], PathAcc extends string[]> = Tree extends readonly (infer Node)[] ? Node extends {
    name: infer Name;
    path: infer Path;
    children?: infer Children;
...
} | ReadRoutes__<Children, [...NameAcc, Name], [...PathAcc, Path]> : never : never : never : never;
/**
...
export declare type ParamsMarker = {
    readonly "@@params": true;
};
export declare type Descend<Path, Tree> = Path extends [infer P1, ...infer PS] ? Tree extends readonly (infer Node)[] ? Node extends {
    name: infer Name;
    children?: infer Children;
} ? Name extends P1 ? Descend<PS, Children> : never : never : never : Tree;
/****************
 * Utility types
 ****************/
